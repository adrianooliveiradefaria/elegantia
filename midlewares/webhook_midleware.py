...
class PostWebhookMiddleware(BaseHTTPMiddleware):
    # A classe BaseHTTPMiddleware espera que o método gerenciador das requisições chame-se dispatch
    async def dispatch(self, request, call_next):
        response = await call_next(request)
        URI = request.url.path  # Path completo da uri; neste caso, que fez o post
        if request.method == 'POST' and request.url.path == URI:
            response_body = b''.join([chunk async for chunk in response.body_iterator])
            response_body_json = json.loads(response_body.decode('utf-8'))
            # Executa o método para envio ao webhook passando o novo cadastro
    ...
    async def enviar_ao_webhook(self, data):
        """
        Método que envia o novo cadastro para o webhook
        """
